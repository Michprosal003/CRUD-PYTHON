...
class UIP:
    def __init__(self, root):
        self.root = root
        self.root.title("Mike Rosales")
        self.root.geometry("1350x700+0+0")

        title = Label(self.root, text="CURSO DE PYTHON", bd=10,
...
# =========MANAGE FRAME=====================================
        ...
        m_title = Label(Manage_frame, text="CURSO DE PYTHON", bg="light blue",
                        fg="black", font=("times new roman", 30, "bold"))
        m_title.grid(row=0, columnspan=2, pady=20)

        lbl_codigo = Label(Manage_frame, text="CODIGO", bg="light blue",
...
# ================TABLE FRAME===============================
    ...
    def add_students(self):
        if self.estudiante_no.get() == "" or self.nombre.get() == "" or self.materia.get() == "" or self.nota.get() == "" or self.jornada.get() == "" or self.estado.get() == "":
            ...
            con = pymysql.connect(
                host="localhost", user="root", password="", database="udelas")
            cur = con.cursor()
